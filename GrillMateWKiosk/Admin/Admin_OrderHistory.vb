    ...
    Private Sub LoadOrders()
        Try
            ...
            If cmbPaymentMethod.SelectedIndex > 0 Then
                parameters.Add(New SqlParameter("@PaymentMethod", cmbPaymentMethod.SelectedItem.ToString()))
            End If

            ' Add order type filter
            If cmbOrderType.SelectedIndex > 0 Then
                parameters.Add(New SqlParameter("@OrderType", cmbOrderType.SelectedItem.ToString()))
            End If

            ' Add order status filter
            If cmbOrderStatus.SelectedIndex > 0 Then
                parameters.Add(New SqlParameter("@OrderStatus", cmbOrderStatus.SelectedItem.ToString()))
            End If

            ' Add date range filters if enabled
            If useDateFilter Then
                parameters.Add(New SqlParameter("@DateFrom", dtpDateFrom.Value.Date))
                parameters.Add(New SqlParameter("@DateTo", dtpDateTo.Value.Date.AddDays(1).AddSeconds(-1)))
            End If
    ...
    Private Function CalculateOrderTotals(orderId As Integer) As Dictionary(Of String, Decimal)
        ...
        Try
            Using conn As SqlConnection = DatabaseConnection.GetConnection()
                ...
                Dim subtotalResult As Object = itemsCmd.ExecuteScalar()
                ...
                Dim paidCmd As New SqlCommand(paidQuery, conn)
                paidCmd.Parameters.AddWithValue("@OrderID", orderId)
                Dim paidResult As Object = paidCmd.ExecuteScalar()
